...
//  在线心跳与新消息检测
// ═══════════════════════════════════════
global.onlineSessions = new Map(); // 全局记录谁在线

app.get('/api/heartbeat/:sessionId', async (req, res) => {
  const sid = req.params.sessionId;
  global.onlineSessions.set(sid, Date.now()); // 记录你最后在线的瞬间
  
  let hasNew = false;
  if (req.query.last_ts) {
    // 偷偷查一下数据库，有没有比前端最新消息还要新的记录（比如 AI 刚刚主动发的一条）
    const { count } = await supabase.from('messages')
      .select('*', { count: 'exact', head: true })
      .eq('session_id', sid)
      .gt('created_at', req.query.last_ts);
    if (count > 0) hasNew = true;
  }
  res.json({ ok: true, hasNew });
});
