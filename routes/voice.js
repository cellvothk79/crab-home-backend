...
app.post('/api/call/stream', async (req, res) => {
  ...
  try {
    ...
    const apiRes = await fetch(apiUrl, {
      ...
      body: JSON.stringify({
        model: useModel, max_tokens: 1024, stream: true,
      ...
      try {
        if (tts_channel === 'elevenlabs' && process.env.ELEVENLABS_API_KEY) {
          ...
          const ttsRes = await fetch(`https://api.minimaxi.com/v1/t2a_v2?GroupId=${process.env.MINIMAX_GROUP_ID||'2067156952080720056'}`, {
            ...
            body: JSON.stringify({
              model: 'speech-02-turbo', text: ttsText, stream: false,
              ...
              language_boost: tts_lang === 'en' ? 'English' : 'Chinese'
            }),
          });
          if (ttsRes.ok) {
...
app.get('/api/call/records', async (req, res) => {
  ...
  const { data, error } = await supabase.from('call_records').select('id, started_at, duration, transcript').eq('session_id', parseInt(session_id)).order('created_at', { ascending: false }).limit(100);
...
app.post('/api/voice/upload', upload.single('audio'), async (req, res) => {
  ...
  try {
    ...
    const { error } = await supabase.storage.from('voice-messages').upload(fileName, req.file.buffer, { contentType: 'audio/webm', upsert: false });
...
app.post('/api/voice/transcribe', upload.single('audio'), async (req, res) => {
  ...
  try {
    ...
    if (!whisperRes.ok) throw new Error('Whisper 失败');
    ...
    try {
      const emotionRes = await fetch('https://api.deepseek.com/v1/chat/completions', {
        ...
        body: JSON.stringify({ model: 'deepseek-chat', max_tokens: 10, temperature: 0, messages: [{ role: 'user', content: `根据这句话判断说话人的情绪，从以下选项选一个：开心、难过、疲惫、撒娇、生气、平静、兴奋。只输出一个词。\n"${text}"` }] }),
      });
...
app.post('/api/voice/tts', async (req, res) => {
  ...
  if (lang === 'en') {
    ...
    } catch(e) { console.log('翻译失败，使用原文:', e.message); }
  }
  ...
  try {
    const ttsRes = await fetch(`https://api.minimaxi.com/v1/t2a_v2?GroupId=${minimaxGroupId}`, {
      method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + minimaxKey },
      body: JSON.stringify({
        model: minimaxModel, text: ttsText, stream: false,
        voice_setting: { voice_id: minimaxVoiceId, speed: 1.0, vol: 1.0, pitch: 0, emotion: emotionToMinimax(emotion) },
        audio_setting: { sample_rate: 32000, bitrate: 128000, format: 'mp3' },
        language_boost: lang === 'en' ? 'English' : 'Chinese',
      }),
    });
    if (!ttsRes.ok) throw new Error('MiniMax TTS 失败');
    const data = await ttsRes.json();
    if (data.base_resp?.status_code !== 0) throw new Error(data.base_resp?.status_msg || 'MiniMax 返回错误');
    const audioBuffer = Buffer.from(data.data?.audio, 'hex');

    if (isCallMode) {
      res.set('Content-Type', 'audio/mpeg'); return res.send(audioBuffer);
    }
    try {
      const fileName = `voice_${Date.now()}.mp3`;
      const { error: uploadErr } = await supabase.storage.from('voice-messages').upload(fileName, audioBuffer, { contentType: 'audio/mpeg', upsert: false });
      if (!uploadErr) {
    ...
    res.set('Content-Type', 'audio/mpeg'); res.send(audioBuffer);
